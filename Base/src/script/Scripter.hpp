...
public:
	...
	public:
		...
		bool exists(const std::string varName) { return host.exists(varName); }
		...
		void pushCommand(const std::string& cmd) { host.commandStack.push_back(cmd); }

		const std::string& operatorTop() const { return host.commandStack.back(); }
		void operatorConsume() { host.commandStack.pop_back(); }

		const std::string& variableNameTop() const { return host.operatorVariables.back().first; }
		void variableNameConsume() { host.operatorVariables.pop_back(); }

		int tempIntTop() const { return host.tempInt.back(); }
		void tempIntConsume() { host.tempInt.pop_back(); }

		float tempFloatTop() const { return host.tempFloat.back(); }
		void tempFloatConsume() { host.tempFloat.pop_back(); }

		const std::string& tempStringTop() const { return host.tempString.back(); }
		void tempStringConsume() { host.tempString.pop_back(); }

		bool tempBoolTop() const { return host.tempBool.back(); }
		void tempBoolConsume() { host.tempBool.pop_back(); }

		void fillParameter(float& f) { host.fillParameterFloat(f); }
		void fillParameter(int& i) { host.fillParameterInt(i); }
		void fillParameter(std::string& s) { host.fillParameterString(s); }
			...
			if (handleCommand(cmd, ss))
				...
				if (!handled) {
					...
					if (functionParameterMapping.back().find(cmd) != functionParameterMapping.back().end()) {
						...
						if (exists(mappedCmd))
							operatorVariables.push_back(std::make_pair(functionParameterMapping.back()[cmd], ++currentPriority));
						else
							process(mappedCmd);
					}
					else
						operatorVariables.push_back(std::make_pair(cmd, ++currentPriority));
				}
            }
        }
    }
...
private:
	...
	void eraseStructVariable(const std::string& type, const std::string& name) {
		...
		for (auto& varTypePair : it->second.nameToType) {
			...
			if (varTypePair.second == "int") { ints.erase(nextName); }
			else if (varTypePair.second == "float") { floats.erase(nextName); }
			else if (varTypePair.second == "string") { strings.erase(nextName); }
			else  { eraseStructVariable(varTypePair.second, nextName); }
		}
	}
	...
	void assignStruct(const std::string& varName1, const std::string& varName2) {
		...
		for (auto& it : structs[type1].nameToType) {
			...
			if (variableType == "int") { assign(ints, varName1 + "." + variableName, varName2 + "." + variableName); }
			else if (variableType == "float") { assign(floats, varName1 + "." + variableName, varName2 + "." + variableName); }
			else if (variableType == "string") { assign(strings, varName1 + "." + variableName, varName2 + "." + variableName); }
			else { assignStruct(varName1 + "." + variableName, varName2 + "." + variableName); }
		}
	}
