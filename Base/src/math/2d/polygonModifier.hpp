...
namespace sa {
	...
	public:
		PolygonModifier(sa::Polygon<T>& polygon);
		...
		PolygonModifier& align(T& pos1, const Scalar& rot1, const T& pos2, const Scalar& rot2, const sa::Polygon<T>& other);
		PolygonModifier& merge(const sa::Polygon<T>& other);

	private:
		Polygon<T>& m_polygon;
		std::vector<T> m_modifiers;

		static std::pair<Scalar, T> minimumDistance(const T& point, const sa::Polygon<T>& polygon, const T& pos, const Scalar& rot);
...
template<typename T> sa::PolygonModifier<T>::PolygonModifier(sa::Polygon<T>& polygon) : m_polygon(polygon) { m_modifiers.resize(polygon.vertices.size()); }

template<typename T> sa::PolygonModifier<T>& sa::PolygonModifier<T>::roundEdges(const Scalar& amount, int iterationCount) {
	typedef decltype(T().x) Scalar;
...
template<typename T> sa::PolygonModifier<T>& sa::PolygonModifier<T>::bend_U(const Scalar& amount) {
	...
	Scalar c = amount * sa::math::PI<Scalar>();
	auto& verts = m_polygon.vertices;
	for (int i = 0; i < static_cast<int>(verts.size()); ++i) {
		auto& v = verts[i];
		Scalar x_trig = c * v.x / top_x;
		
		sa::vec3<Scalar> rot_x(v.x, Scalar(0), Scalar(0));
		sa::math::rotateXY(rot_x, -x_trig);

		sa::vec3<Scalar> rot_addition(Scalar(0), v.y, Scalar(0));
		sa::math::rotateXY(rot_addition, -x_trig * Scalar(2));
...
template<typename T> sa::PolygonModifier<T>& sa::PolygonModifier<T>::bend_S(const Scalar& amount) {
	Scalar top_x = -Scalar(1000);
	for (auto& v : m_polygon.vertices) {
		if (v.x > top_x)
			top_x = v.x;
...
template<typename T> sa::PolygonModifier<T>& sa::PolygonModifier<T>::align(T& pos1, const Scalar& rot1, const T& pos2, const Scalar& rot2, const sa::Polygon<T>& other) {
	...
	for (int i = 0; i < 4; ++i) {
		T mod = sa::math::rotatedXY(v1, rot1) + sa::math::rotatedXY(v2, rot1) - sa::math::rotatedXY(matchingVertices.first, rot2) - sa::math::rotatedXY(matchingVertices.second, rot2) + pos1 * Scalar(2) - pos2 * Scalar(2);
		mod /= Scalar(2);
		for (auto& v : vertices) {
			pos1 += mod;
		}
...
template<typename T> sa::PolygonModifier<T>& sa::PolygonModifier<T>::merge(const sa::Polygon<T>& other) {
	// find a pair of vertices that are closest to a pair on the other polygon.

	// enumerate all vertices in proper order to create the merged polygon.
}

template<typename T> std::pair<decltype(T().x), T> sa::PolygonModifier<T>::minimumDistance(const T& point, const sa::Polygon<T>& polygon, const T& pos, const Scalar& rot) {
	Scalar minDist(10000);
	T t;
	for (const auto& v : polygon.vertices) {
		Scalar dist = (sa::math::rotatedXY(v, rot) + pos).distanceSquared(point);
		if (dist < minDist) {
			minDist = dist;
			t = v;
		}
